...
class Histogram extends Frame
  ...
  Histogram(){
    ...
    brightField = new TextField("1.5",5);
  ...
  public int[][][] processImg(
    ...
    newHistogram = changePix(output3D,
                                imgRows,imgCols);
  ...
  //Method to change the four least significant bits in each color to zero before sending to histogram
  int[] changePix(int[][][] data3D,int imgRows,int imgCols){
  
    int[] hist = new int[256];
    for(int row = 0;row < imgRows;row++){
      for(int col = 0;col < imgCols;col++){
        //Set last four bits to 0
        for(int i = 0; i < 4; i++){
          data3D[row][col][1] &= ~(1 << i);
          data3D[row][col][2] &= ~(1 << i);
          data3D[row][col][3] &= ~(1 << i);
        }
        hist[data3D[row][col][1]]++;
        hist[data3D[row][col][2]]++;
        hist[data3D[row][col][3]]++;
